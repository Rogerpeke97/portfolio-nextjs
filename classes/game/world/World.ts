...
export class World {
  ...
  private addPlanet() {
    ...
    planet.position.set(this.CENTER_ORIGIN_AXES.x, this.CENTER_ORIGIN_AXES.y, this.CENTER_ORIGIN_AXES.z)
  ...
  private calculateParticleRotation = (vector: THREE.Vector3, indexInArray: number, particlePositions: (THREE.BufferAttribute | THREE.InterleavedBufferAttribute), rotationAngle: number) => {
    ...
    const particleAfterRotationZ = Math.cos(rotationAngleInRadians) * (vector.z - this.CENTER_ORIGIN_AXES.z) - Math.sin(rotationAngleInRadians) * (vector.y - this.CENTER_ORIGIN_AXES.y) + this.CENTER_ORIGIN_AXES.z
    const particleAfterRotationY = Math.sin(rotationAngleInRadians) * (vector.z - this.CENTER_ORIGIN_AXES.z) + Math.cos(rotationAngleInRadians) * (vector.y - this.CENTER_ORIGIN_AXES.y) + this.CENTER_ORIGIN_AXES.y
  ...
  private addAmbientParticles () {
    ...
    for (let i = 0; i < this.PARTICLES_COUNT; i++) {
      particlePositionX = (Math.random() * this.PARTICLES_DISTANCE) + this.CENTER_ORIGIN_AXES.x / 2 - 10
      particlePositionY = (this.CENTER_ORIGIN_AXES.y - this.PARTICLES_DISTANCE / 2) + (Math.random() * this.PARTICLES_DISTANCE)
      particlePositionZ = Math.random() * this.PARTICLES_DISTANCE
