...
export class World {
  ...
  private addPlanet() {
    ...
    planet.position.set(
      this.PLANET_CENTER.x,
      this.PLANET_CENTER.y,
      this.PLANET_CENTER.z
    );
  ...
  private calculateParticleRotation = (
    vector: THREE.Vector3,
    indexInArray: number,
    particlePositions: THREE.BufferAttribute | THREE.InterleavedBufferAttribute,
    rotationAngle: number
  ) => {
    const rotationAngleInRadians = (rotationAngle * Math.PI) / 180;
    const particleAfterRotationZ =
      Math.cos(rotationAngleInRadians) * (vector.z - this.PLANET_CENTER.z) -
      Math.sin(rotationAngleInRadians) * (vector.y - this.PLANET_CENTER.y) +
      this.PLANET_CENTER.z;
    const particleAfterRotationY =
      Math.sin(rotationAngleInRadians) * (vector.z - this.PLANET_CENTER.z) +
      Math.cos(rotationAngleInRadians) * (vector.y - this.PLANET_CENTER.y) +
      this.PLANET_CENTER.y;
    const particleAfterRotationX = vector.x;
    particlePositions.setXYZ(
      indexInArray,
      particleAfterRotationX,
      particleAfterRotationY,
      particleAfterRotationZ
    );
  };
  private moveParticles() {
    const particlesSystem = this.scene.getObjectByName(
      "particlesSystem"
    ) as THREE.Points;
    //@ts-ignore
    if (particlesSystem.material?.uniforms) {
      //@ts-ignore
      particlesSystem.material.uniforms.time.value =
    ...
    for (let i = 0; i < this.PARTICLES_COUNT; i++) {
      ...
      vectorCollision.set(
        particlesPosition.getX(i),
        particlesPosition.getY(i),
        particlesPosition.getZ(i)
      );
      if (this.hasCollidedWithPlanet(vectorCollision)) {
        trackedParticle = this.trackedStillParticles.find(
          (particle) => particle.index === i
        );
        if (trackedParticle) {
          if (this.clock.getElapsedTime() - trackedParticle.wasSetAt > 2) {
            this.trackedStillParticles.splice(
              this.trackedStillParticles.indexOf(trackedParticle),
              1
            );
            particlesPosition.setY(
              trackedParticle.index,
              particlesPosition.getY(trackedParticle.index) + 30
            );
            continue;
          }
          this.calculateParticleRotation(
            vectorCollision,
            i,
            particlesPosition,
            trackedParticle.angleOfRotation
          );
        ...
        this.trackedStillParticles.push({
          index: i,
          position: vectorCollision,
          wasSetAt: this.clock.getElapsedTime(),
          angleOfRotation: 0,
        });
        continue;
      }
      particlesPosition.setY(i, particlesPosition.getY(i) - 0.01);
  ...
  private trackWindowAndContainerResize() {
    ...
    this.observerOfCanvasContainer = new ResizeObserver(
      this.updateRendererAndCamera.bind(this)
    );
    this.observerOfCanvasContainer.observe(
      document.getElementById("gameSceneContainer") as HTMLElement
    );
  }
  private setCameraPositionAndAspect() {
    ...
    this.camera.aspect =
      this.canvasContainer.width / this.canvasContainer.height;
  ...
  private setupIllumination() {
    ...
    directionalLight.position.set(
      this.PLANET_CENTER.x,
      this.PLANET_CENTER.y + this.CENTER_OBJECT_RADIUS * 2,
      this.PLANET_CENTER.z - 8
    );
  ...
  private addAmbientParticles() {
    ...
    for (let i = 0; i < this.PARTICLES_COUNT; i++) {
      particlePositionX =
        Math.random() * this.PARTICLES_DISTANCE - this.PLANET_RADIUS * 2.5;
      particlePositionY =
        Math.random() * this.PARTICLES_DISTANCE - this.PLANET_RADIUS * 2.5;
      particlePositionZ =
        Math.random() * this.PARTICLES_DISTANCE - this.PLANET_RADIUS * 2.5;
      ...
      color.set(
        Math.random() * NORMALIZED_VEC_MAX,
        Math.random() * NORMALIZED_VEC_MAX,
        Math.random() * NORMALIZED_VEC_MAX
      );
    ...
    const particlesSystem = new THREE.Points(
      this.particles,
